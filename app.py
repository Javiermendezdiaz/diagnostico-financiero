"""ITAP — Backend de produccion (hardened). Instrumento + motor + Libro Financiero. Profundidad por tier."""
...
def generar_pdf(sid, email, respuestas, datos, tier):
    ...
    rb.build_book(respuestas, datos_completos(datos), cli, out, depth=TIER_DEPTH.get(tier, "completo"))
    return out

@app.get("/")
def health():
    return {"status": "healthy", "service": "ITAP", "capas": len(rb.INST["capas"]),
            "version": rb.INST["meta"]["version"], "persistencia": "sqlite", "tiers": "esencial/completo"}
...
def start(payload: StartPayload):
    ...
    with db() as c:
        c.execute("INSERT INTO sesiones(id,email,tier,respuestas,datos,creado) VALUES(?,?,?,?,?,?)",
                  (sid, payload.email, payload.tier, "{}", "{}", datetime.datetime.utcnow().isoformat()))
...
def report(session_id: str):
    ...
    if not os.path.exists(path):
        with db() as c:
            row = c.execute("SELECT email,tier,respuestas,datos FROM sesiones WHERE id=?", (session_id,)).fetchone()
